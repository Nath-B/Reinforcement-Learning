function [Arms,Rewards] = LinUCB_SIN(nb_arms,url_contexts,user_urls,alpha,T,unique_urls,bool_random_gaussian)
% Implements the LinUCB_SIN algorithm (a single cluster)
...
% Initialize single-cluster parameters
size_cluster = n_users;
M_clust = eye(d)+sum(M,3) - size_cluster*eye(d);
b_clust = sum(b,2);
w_clust = M_clust\b_clust;
...
for t=1:T
    ...
    for a=1:nb_arms
        UCB(a) = w_clust'*contexts(a,:)' + alpha*sqrt(contexts(a,:)*((M_clust\(contexts(a,:)'))));
    end
    % Define the best arm
    [~,Arms(t)] = max(UCB);
    % Define the reward
